...
function parseScoring(marksValue, negativeMarksValue) {
    const marks = marksValue === undefined || marksValue === '' ? 1 : Number(marksValue);
    const negativeMarks = negativeMarksValue === undefined || negativeMarksValue === '' ? 0 : Number(negativeMarksValue);
    if (!Number.isFinite(marks) || marks <= 0) {
        throw new Error('marks must be a number greater than 0');
    }
    if (!Number.isFinite(negativeMarks) || negativeMarks < 0) {
        throw new Error('negative_marks must be a number of 0 or greater');
    }
...
router.post('/bulk', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const validTopics = new Map(topicsRes.rows.map((topic) => [topic.id, topic]));

            if (validTopics.size === 0) {
                await client.query('ROLLBACK');
                return res.status(400).json({
                    error: 'This exam has no topics yet. Create a subject and topic in Question Bank before importing questions.',
                });
            }
            ...
            for (let i = 0; i < questionList.length; i++) {
                ...
                try {
                    scoring = parseScoring(q.marks, q.negative_marks);
                } catch (err) {
                    failed.push({ index: i, item: q, error: err.message });
                    continue;
                }
                ...
                if (!validTopics.has(topicId)) {
                    failed.push({
                        index: i,
                        item: q,
                        error: `topic_id ${q.topic_id} does not exist in the active exam. Use a topic ID shown in Question Bank.`,
                    });
                    continue;
                }
                ...
                const ins = await client.query(
                    `INSERT INTO questions 
                        (exam_id, topic_id, difficulty, body, option_a, option_b, option_c, option_d, correct_opt, explanation, image_url, marks, negative_marks)
            ...
            res.json({
                ...
                failed,
                availableTopics: topicsRes.rows,
            });
        } catch (err) {
            await client.query('ROLLBACK');
            throw err;
...
router.post('/', upload.single('image'), async (req, res) => {
    ...
    let scoring;
    try {
        scoring = parseScoring(marks, negative_marks);
    } catch (err) {
    ...
    try {
        ...
        try {
            scoring = parseScoring(
                marks === undefined ? existing.rows[0].marks : marks,
                negative_marks === undefined ? existing.rows[0].negative_marks : negative_marks
            );
        } catch (err) {
            return res.status(400).json({ error: err.message });
        }
