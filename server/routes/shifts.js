...
router.post('/', async (req, res) => {
    const { exam_id, name, scheduled_start } = req.body;
    if (!name || !name.trim()) {
    ...
    try {
        ...
        const result = await pool.query(
            `INSERT INTO shifts (exam_id, name, scheduled_start)
             VALUES ($1, $2, $3)
             RETURNING *`,
            [targetExamId, name.trim(), scheduled_start ? new Date(scheduled_start) : null]
        );
        ...
        try {
            ...
            for (const sess of unsubmitted.rows) {
                ...
                await client.query(
                    ...
                    [sess.candidate_id, id, correct, total, correct, wrong, skipped, sess.active_seconds || 0, now]
                );
