...
export default function ExamPage() {
                ...
                <div className="max-w-md w-full bg-[#140b0b] border border-green-500/40 rounded-2xl p-8 text-center shadow-2xl space-y-6 relative overflow-hidden">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                            Your responses have been successfully submitted and securely stored in the database.
                        </p>
                    </div>
                    ...
                    <div className="p-4 bg-amber-500/10 border border-amber-500/25 rounded-xl text-center space-y-1">
                        ...
                        <p className="text-xs text-gray-300 leading-relaxed">
                            The results for the examination you wrote will be revealed soon.
                        </p>
                    </div>
    ...
    return (
        <div className="min-h-screen bg-[#0d0707] text-[#fbe9e7] flex flex-col font-sans selection:bg-orange-500 selection:text-white select-none">
            {/* Proctoring Security Warning Overlay / Fullscreen Gate */}
            {showWarning && (
                <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className={`border rounded-2xl max-w-md w-full p-8 shadow-2xl text-center space-y-5 ${
                        isViolation ? 'bg-[#1c0a0a] border-red-500/50' : 'bg-[#180d0d] border-orange-500/40'
                    }`}>
                        <span className={`material-symbols-outlined text-5xl ${isViolation ? 'text-red-400' : 'text-orange-400'}`}>
                            {isViolation ? 'security' : 'fullscreen'}
                        </span>
                        ...
                        {isViolation && violationCount > 0 && (
                            <div className="inline-block px-3 py-1 bg-red-900/40 border border-red-500/30 rounded-full text-xs font-mono text-red-300">
                                Violations: {violationCount} / {maxViolations || 100}
                            </div>
                        )}
                        <button
                            onClick={warningAction}
                            className={`w-full py-3.5 text-white font-bold rounded-xl transition cursor-pointer shadow-lg ${
                                isViolation
